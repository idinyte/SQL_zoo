...
SELECT count(*) AS no_stops FROM stops
...
SELECT DISTINCT a.num, a.company, stop.name, b.num, b.company
FROM
(
...
(
	...
	FROM stops
	WHERE name = 'Lochend') 
) AS b ON (a.stop = b.stop)
JOIN stops AS stop ON (a.stop = stop.id)
ORDER BY a.num, stop.name, b.num