...
SELECT 
  ...
  RANK() OVER (ORDER BY (confirmed/population)) AS infect_rate
...
FROM
    (SELECT name, date, new, RANK() OVER(PARTITION BY name ORDER BY new DESC) AS rank 
        FROM(
            SELECT name, DATE_FORMAT(whn,'%Y-%m-%d') AS date, confirmed - LAG(confirmed, 1) OVER (PARTITION BY name ORDER BY whn) AS new
            FROM covid
            ) TAB
        WHERE new > 1000
    ) TAB
